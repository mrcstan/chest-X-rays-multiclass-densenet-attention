...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.att_est1 = nn.Conv2d(in_channels=(128+1024), out_channels=1, kernel_size=1, stride=1, padding=0, bias=False)
        self.att_est2 = nn.Conv2d(in_channels=(256+1024), out_channels=1, kernel_size=1, stride=1, padding=0, bias=False)
        self.att_est3 = nn.Conv2d(in_channels=(512+1024), out_channels=1, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def calculate_ga(self, l, g, att_est):
        ...
        # cat(l, g).shape = (num_batch, num of l channels+num_of g channels, num_pixels_row, num_pixels_col)
        g_expand = g.expand(g.shape[0], g.shape[1], l.shape[2], l.shape[3])
        #print('g_expand.shape = ', g_expand.shape)
        #print('l.shape = ', l.shape)
        c = att_est(torch.cat((l,g_expand), dim=1))
        # c.shape = (l.shape[0], 1, l.shape[2], l.shape[3])
...
#       features = self.features(x)
        ...
        #g_all = torch.cat((ga1, ga2, ga3, g), dim=1)
...
def densenet121attB(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
